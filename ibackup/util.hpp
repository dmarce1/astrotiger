...
struct IsIntegralPair<std::pair<A, B>> {
	static constexpr bool value = std::is_integral<A>::value && std::is_integral<B>::value;
};

template<typename A, typename B>
struct IsIntegralPair<std::tuple<A, B>> {
	static constexpr bool value = std::is_integral<A>::value && std::is_integral<B>::value;
};


template<typename A>
struct IsIntegralPair<std::array<A, 2>> {
	static constexpr bool value = std::is_integral<A>::value;
};
...
#define assert(cond)                               \
   do {                                            \
      if (!(cond)) {                               \
    	 std::string str;                          \
    	 str += std::string("Assertion failed: "); \
    	 str += #cond;                             \
    	 str += " at ";                            \
    	 str += __FILE__;                          \
    	 str += ":";                               \
