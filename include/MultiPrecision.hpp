...
struct MultiPrecision {
	...
	friend MultiPrecision assoc_legendre(int l, int m, MultiPrecision const &x) {
	    using MP = MultiPrecision;
	    MP one(1), two(2);
	    MP pmm = one;
	    if (m > 0) {
	        MP somx2 = sqrt(one - sqr(x));
	        MP fact = one;
	        for (int i = 1; i <= m; i++) {
	            pmm *= -fact * somx2;
	            fact += two;
	    ...
	    MP pmmp1 = x * MP(2*m + 1) * pmm;
	    if (l == m+1) {
	        return pmmp1;
	    }
	    MP pll = 0;
	    for (int n = m + 2; n <= l; n++) {
	        pll = (MP(2*n - 1) * x * pmmp1 - MP(n + m - 1) * pmm) / MP(n - m);
	        pmm = pmmp1;
	        pmmp1 = pll;
	    }
	    return (m & 1) ? -pll : pll;
	...
	friend MultiPrecision laguerre(int n, MultiPrecision const &x) {
		MultiPrecision p2, p1, p0;
		p0 = MultiPrecision(0);
		p1 = MultiPrecision(1);
		for (int k = 0; k < n; k++) {
			p2 = (MultiPrecision(2 * k + 1 - x) * p1 - MultiPrecision(k) * p0) / MultiPrecision(k + 1);
			p0 = p1;
			p1 = p2;
		}
		return p1;
